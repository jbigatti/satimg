...
if __name__ == "__main__":
    ...
    try:
        shapefiles = [
            os.path.join('real_data/150201/test', "%s.shp" % c) for c in classes]
    except OSError.FileNotFoundError as e:
        report_and_exit(str(e))
    ...
    labels = [x.split('_')[0][:3] for x in classes]
    print_cm(metrics.confusion_matrix(verification_labels, predicted_labels), labels)
    ...
    if validation_data_path:
        ...
        try:
            shapefiles = [
                os.path.join(validation_data_path, "%s.shp" % c) for c in classes]
        except OSError.FileNotFoundError as e:
            report_and_exit(str(e))
